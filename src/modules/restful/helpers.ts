...
async function echoApiDocs(name: string, doc: APIDocOption, appUrl: string) {
  ...
  if (doc.default) {
    console.log(
      `      default: ${chalk.green.underline(getDocPath(doc.default.path))}`,
    );
  }
  if (doc.routes) {
 ...
 * @param version
 */
export const genDocPath = (
  routePath: string,
  prefix?: string,
  version?: string,
) =>
  trimPath(
    `${prefix}${version ? `/${version.toLowerCase()}/` : '/'}${routePath}`,
    false,
  );
