...
import { SeedHandler } from '../helpers';
 ...
 * 数据填充
 */
export const SeedCommand: CommandItem<any, SeederArguments> = async ({
  configure,
}) => ({
  command: ['db:seed', 'dbs'],
  ...
  handler: async (args: Arguments<SeederArguments>) =>
    SeedHandler(configure, args),
});
